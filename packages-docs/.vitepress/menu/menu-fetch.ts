...
export default async function menuVue(): Promise<IMenuRules | undefined> {
  const packagesDir = path.resolve(__dirname, `../../src/${EOutDir.FETCH}` );  // 根据需要设置路径
  ...
  if (fs.existsSync(packagesDir)) {
    try {
      ...
      return {
        nav: {
          text: "Fetch",
          link: `/src/${EOutDir.FETCH}/${filesWithoutMd[0]}`,
          activeMatch: `/src/${EOutDir.FETCH}/`
        },
        menu: filesWithoutMd.map(item => ({
          text: item,
          link: `/src/${EOutDir.FETCH}/${item}`
        }))
      };
    } catch (error) {
