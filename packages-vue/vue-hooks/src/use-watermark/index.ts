...
function findTargetNode(el: unknown): Omit<TUseWatermarkRes, "clearAll"> | undefined {
  return Array.from(sourceMap.values()).find(item => item.targetElement === el);
}

function createBase64(str: string, waterMarkOptions: TWaterMarkOptionsType): string {
...
export default function useWatermark(
  ...
  if (appendElRaw && sourceMap.has(domSymbol)) {
    const {
      setWatermark, clear
    } = sourceMap.get(domSymbol) as TUseWatermarkRes;
